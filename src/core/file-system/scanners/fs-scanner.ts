import { isNonEmptyStringArray } from '@shared/guards';
import { FileSystemEntry, FileSystemEntryFactory } from '../entries';
import { ScannerOptions } from './types';
import type { GlobEntry } from '../entries/types';
...
export default class FileSystemScanner {
  ...
  private async *scanEntries(source: string | string[], options: ScannerOptions = {}) {
    ...
    for await (const entry of stream as AsyncIterable<GlobEntry>) {
      yield entry;
    }
  }

  async scan(source: string | string[], options: ScannerOptions = {}): Promise<FileSystemEntry[]> {
