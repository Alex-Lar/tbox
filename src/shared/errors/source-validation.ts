...
export class SourceValidationError extends Error implements PrettyError {
    ...
    constructor(invalidPaths: InvalidPathsCollection) {
        const totalCount = invalidPaths.emptyDirs.length + invalidPaths.invalidPaths.length;
    ...
    private generateSolution(): string {
        ...
        if (emptyDirs.length > 0) {
            ...
            solutions.push(`  ${BULLET_SYMBOL} Use --force to skip empty directories`);
        }

        if (invalidPaths.length > 0) {
            solutions.push(`  ${BULLET_SYMBOL} Verify invalid paths exist and are accessible`);
